        ...
        if role_add_flag:
            ...
            if err_announce is not None:
                ...
                if len(results_name) == 0:
                    ...
                    notion_update_filter = [{"ユーザーID": str(message.author.id), "講師番号": instructor_id_key, "受講コース": "Java"}]
                    ...
                    for message in messages:
                        # await message.delete()
                        if not (message.id == 1232217142410219591):
                            await message.delete()
...
"""
def create_notion_instructor_info(message):

    notion = Notion()

    display_name = message.author.display_name
    username = display_name.split("_")[0]
    database_key = display_name.split("_")[1]
    # database_id = database_id_list[database_key]
    ...
    for key, val in instructor_id_list.items():
        print(key, val)
        if key != "講師DB":
            results = notion.select(database_id=val["id"], filter_dict={'在籍状況': '受講中'})
